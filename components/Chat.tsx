...
export default function Chat() {
  ...
  // === ГЛАВНОЕ: защита от двойной отправки ===
  ...
  const enterToSubmit = (e: React.KeyboardEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      (e.currentTarget.form as HTMLFormElement | null)?.requestSubmit(); // запуск onSubmit формы
    }
  };

  // Единый обработчик отправки (только через onSubmit формы)
  const handleSend = useCallback(
    async (e?: React.FormEvent<HTMLFormElement>) => {
      if (e) e.preventDefault();
      if (sendingRef.current) return;          // уже отправляем → выходим

      const text = input.trim();
      if (!text) return;

      // Локально показываем сообщение юзера
      ...
      try {
        // Вызов твоего API. Сигнатуры у всех разные, поэтому читаем универсально.
        // Если у тебя стрим — сервер всё равно отдаст финальный ответ или ошибку.
        const res = await fetch('/api/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            messages: [...messages, userMsg], // контекст на сервер (если нужно)
            input: text,                      // на случай другой сигнатуры
            message: text                     // дублируем ключи для совместимости
          }),
        });
        ...
        setIsSending(false);
        sendingRef.current = false; // обязательно сбрасываем флаг
      }
    },
    [input, messages]
  );
  ...
  // Микрофон (Web Speech API) — опционально
  ...
  return (
    <div className="flex h-[100dvh] flex-col bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      {/* Верхняя панель — как у тебя была (текст оставляю) */}
      ...
      {/* Панель ввода снизу. ВАЖНО: только onSubmit! Кнопка — type="submit". */}
      <form onSubmit={handleSend} className="border-t border-zinc-200 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mx-auto flex w-full max-w-[880px] items-center gap-2">
          ...
          {/* Стрелка «Отправить» — НЕТ onClick, ТОЛЬКО type="submit" */}
          <button
            type="submit"
            disabled={isSending}
